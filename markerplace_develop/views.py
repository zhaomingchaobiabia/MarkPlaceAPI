...
def pricing_query(request):
    if request.method == 'GET':
        return render(request, 'pricing_query.html')
    if request.method == 'POST':
        data = request.POST
        ...
        return render(request, 'pricing_query.html', {'data': data})


@csrf_exempt
def messages_query(request):
    if request.method == 'GET':
        return render(request, 'messages_query.html')
    if request.method == 'POST':
        data = request.POST
...
def messages_query_type(request):
    if request.method == 'POST':
        ...
        data_dict = {'message_type': data.get('message_type'), 'message_archived': data.get('message_archived'),
                     'message_state': data.get('message_state'), 'sort_by': data.get('sort_by'),
